...
& $msbuild (Join-Path $repoRoot 'tests/CommonCompatibility/CommonCompatibility.csproj') /p:Configuration=Release "/p:TargetFrameworkRootPath=$referenceRoot\" /verbosity:minimal
if ($LASTEXITCODE -ne 0) { throw 'Compatibility test compilation failed.' }
& (Join-Path $repoRoot 'tests/CommonCompatibility/bin/Release/CommonCompatibility.exe')
