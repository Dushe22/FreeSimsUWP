...
## 2026-09-23 - Real TS1 content and isolated save probe

Installation confirmed at the external intake folder: 90 FAR archives, 81 houses in UserData; layout/header check PASS. No game assets added to Git or packages.
Fixed two observed parser failures: NGBH permits omitted inventory data; OBJD v136 ends before BHAV_Repair. Also corrected NGBH serialized inventory-owner count, verified by synthetic roundtrip. Cross-check: https://github.com/riperiperi/FreeSO/blob/master/TSOClient/tso.files/Formats/IFF/Chunks/NGBH.cs and OBJD.cs (reference only, no upstream merge).
Added shared NeighborhoodStore overlay: read originals until a save exists, write only under UserData/Neighborhoods, temporary-file replacement and failed-write cleanup. This helper is exercised by the probe; legacy TS1NeighborhoodProvider and full game startup are not connected yet. Reparse-point aliases are outside the helper's current path validation contract; use ordinary isolated app folders.
Desktop real-data checks: 9/9 PASS; 49 neighbors, 24 recognized House01 chunks, 126 object IFFs / 743 definitions from Objects.far. All three input SHA256 hashes unchanged. Synthetic files/layout/parser regressions, common 14/14 and desktop client Release build PASS. This does not validate every archive, resource decoder or gameplay save serialization.
New asset-free XboxContentProbe 0.1.0.0, identity Dushe22.FreeSimsXboxContentProbe, pinned MonoGame 3.8.1.303. Release/x64 .NET Native compile/package PASS, with no ILT/MCG warnings. Signed handoff validation and Series S results follow when available.
Hardware procedure: experiments/XboxContentProbe/TESTING.md. Upload only Objects.far, Neighborhood.iff and House01.iff to its LocalState/GameData with the original relative layout (GameData/GameData/Objects is intentional). A reruns, B exits; require 9/9 after repeat, Home/return and relaunch. Return photo and its LocalState/proof.log. Test saves are unique scratch copies under UserData/ContentProbe. No new game-data zip is produced.
Next gate: real-file/native reflection and File.Replace behavior on Series S. After PASS, connect the isolated neighborhood store and implement the offline TS1 startup/VM path; full lot rendering and simulation remain unverified. Previous Xbox-verified probes are retained.
Signed content handoff: source e0d1abbcc9b1842f60a467818d2eb8ddd507f935; native Release/x64 PASS. AppX SHA256 5E20E0BE88E523D467D58176AD4E042110AA2C8D4CA648D1C4C6B0639ADFBE69. ZIP artifacts/xbox-content-test-e0d1abbcc9b1.zip SHA256 317C1BE033D568C8A1CCFE8FDD01CC7F1D8C7140719663C021CBB9FC058B986B. Audit PASS: identity/version, x64 PE, 87 block hashes, CMS signature-only validation and matching public certificate 91FA3869D973DF0DD50BD6643F9AB182CB3CD025. ZIP has seven files: app, three x64 dependencies, public certificate, BUILD.txt and TESTING.md; no game data/private key. Series S execution remains pending. Latest source changes are parser fixes, NeighborhoodStore, tests, ContentProbe and build selector; previous runtime behavior retained except the shared parser fixes.

## 2026-09-26 - Offline VM and neighborhood preview
Hardware checkpoint: user reported Content Probe 9/9 PASS on Series S; IMG_20260923_104103.jpg shows source e0d1abbcc9b1, run 2, viewport/UI 1280x720. Earlier 5/9 log was missing uploaded UserData files; no newer log supplied.

Added VMOfflineDriver: authoritative local queue, Verify-before-Execute, deferred reentrant commands, network-origin rejection and shutdown. Moved transport OnPacket into VMNetworkDriver; desktop client/server and UIPacketHandlers use that adapter. VM authority no longer depends on a concrete server type. Network gameplay is compile-checked, not retested.
Headless architecture no longer touches roof/lighting render objects when VM.UseWorld=false. Desktop test advances the actual VM/architecture/clock 150 ticks on a synthetic EMPTY 8x8 lot; no populated lot or object behavior is claimed.
TS1NeighborhoodProvider has an explicit GamePaths/zero-based-neighborhood constructor. It reads through NeighborhoodStore, honors saved house overrides and keeps its active state if selection fails. This constructor is a READ-ONLY PREVIEW: complete neighborhood/house serialization is not validated, so both legacy save entry points throw before modifying files. The old Content-based constructor remains unchanged. Full gameplay saving still needs NBRS/unknown-chunk preservation work.
Desktop: client Release build PASS; offline suite 8/8, real house thumbnail CPU decode 320x172, content regression 9/9 PASS; tested input hashes unchanged. Missing temporary .NET 4.5 reference files were restored from the local NuGet cache. Reproduce with scripts/Test-OfflineCompatibility.ps1 -NuGetPath <nuget.exe>.
New XboxOfflineProbe 0.1.0.0: Release/x64 .NET Native compile/package PASS without ILT/MCG warnings. Adds native thumbnail decode/readback for 9 checks total and displays House 1. No Sims assets packaged. See experiments/XboxOfflineProbe/TESTING.md: upload only Neighborhood.iff and House01.iff into this NEW app's GameData/UserData tree; A reruns, B exits; require 9/9 plus thumbnail after repeat/Home/relaunch. Prior probes are retained.
Next gate: Series S offline/preview execution. Then isolate TS1 content initialization from TSO providers, load populated lots and audit full save serialization before enabling saves. Existing VMWorldActivator assumes TSO assets/providers and needs further work; this checkpoint does not boot full gameplay.
Signed offline handoff: source afd89cb154d00b62ada414c85fc5368ef6381f19; AppX SHA256 A34EA07D34A3F99CB22B2A647EABA113A7C7FB4F881CA10959CBA7599818376F. ZIP artifacts/xbox-offline-test-afd89cb154d0.zip SHA256 4424F37CCD3FDE2A9EDB1C8D8883AA0D7B337D53B7987BEB8E1CCD2B6A429F32. Audit PASS: x64 identity/PE, 240 block hashes, CMS signature-only and matching public certificate D1770E0AFC54FD35AE1237DA914D40D61AD2FE09. Seven handoff files (app, three dependencies, public certificate, BUILD.txt, TESTING.md); no copyrighted data or private key. Signed Release/x64 native build PASS; Xbox execution remains pending. Stop here for 9/9 and visible House 1 thumbnail on Series S.
